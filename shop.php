...
$wishlist_count = $row_wish['total'];
        ...
        <div class="max-w-7xl mx-auto px-4 mt-6">
            <main>
                <h2 class="text-lg sm:text-xl md:text-2xl font-bold mb-6">All Products</h2>
...
if(isset($_GET['id']) && !empty($_GET['id'])){
    ...
    $select_category_product = mysqli_query($connect,"SELECT * FROM `sub-category` WHERE status='1' AND id='$id'");
    $row_category_product = mysqli_fetch_assoc($select_category_product);

 
        $sub_category = $row_category_product['name_parentname'];

        $select_cate_pr = mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND subcategory='$sub_category' AND discount_type='0' ");
        while($row_cate_pr = mysqli_fetch_assoc($select_cate_pr)){
            ?>
                    <?php
          $product_id=$row_cate_pr['id'];
          ...
          ?>
                    ...
                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_cate_pr['id']?>">
                            <img src="images/<?php echo $row_cate_pr['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_cate_pr['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR:
                                <?php echo $row_cate_pr['price']?> </p>
                        </a>
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_cate_pr['id']; ?>">
                                <button
                    ...
                    <!--fixed-->
                    <?php }
          $select_cate_pr1 = mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND subcategory='$sub_category' AND discount_type='1' ");
        while($row_cate_pr1 = mysqli_fetch_assoc($select_cate_pr1)){
            ?>
                    <?php
          $product_id=$row_cate_pr1['id'];
              $check = mysqli_query($connect, "SELECT * FROM wishlist WHERE email='".$_SESSION['email']."' AND product_id='$product_id'");
              $inWishlist = mysqli_num_rows($check) > 0;
          ?>

                    <?php
            $price=$row_cate_pr1['price'];
            $per=$row_cate_pr1['discount_value'];
            $dis=($price*$per)/100;
            $final_pr=$price-$dis;
            ?>
                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_cate_pr1['id']?>">
                            <img src="images/<?php echo $row_cate_pr1['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_cate_pr1['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR: <?php echo $final_pr;?> <span
                                    class="line-through text-gray-400 text-xs">PKR-<?php echo $row_cate_pr1['price']?></span><span
                                    class="ml-2 bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-sm">-<?php echo $row_cate_pr1['discount_value']?>%</span>
                            </p>
                        </a>
                        <div class="absolute top-2 right-2 flex flex-col space-y-2">
                            <form method="POST" action="wishlist-add.php">
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_cate_pr1['id']; ?>">
                                <button
                                    class="bg-white p-2 rounded-full shadow <?php echo $check_cart_c?'text-orange-600' : 'text_black hover:text-orange-600';?>"><i
                                        class="fas fa-cart-plus"></i></button>
                            </form>
                    ...
                    <!--per-->
                    <?php }
          $select_cate_pr2 = mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND subcategory='$sub_category' AND discount_type='2' ");
        while($row_cate_pr2 = mysqli_fetch_assoc($select_cate_pr2)){
            ?>
                    <?php
          $product_id=$row_cate_pr2['id'];
              $check = mysqli_query($connect, "SELECT * FROM wishlist WHERE email='".$_SESSION['email']."' AND product_id='$product_id'");
              $inWishlist = mysqli_num_rows($check) > 0;
          ?>

                    <?php
            $price=$row_cate_pr2['price'];
            $per=$row_cate_pr2['discount_value'];
            
            $final_pr=$price-$per;
            ?>
                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_cate_pr2['id']?>">
                            <img src="images/<?php echo $row_cate_pr2['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_cate_pr2['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR: <?php echo $final_pr;?> <span
                                    class="line-through text-gray-400 text-xs">PKR-<?php echo $row_cate_pr2['price']?></span><span
                                    class="ml-2 bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-sm">PKR-<?php echo $row_cate_pr2['discount_value']?></span>
                            </p>
                        </a>
                        <div class="absolute top-2 right-2 flex flex-col space-y-2">
                            <form method="POST" action="wishlist-add.php">
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_cate_pr2['id']; ?>">
                                <button
                                    class="bg-white p-2 rounded-full shadow <?php echo $check_cart_c?'text-orange-600' : 'text_black hover:text-orange-600';?>"><i
                                        class="fas fa-cart-plus"></i></button>
                            </form>
  ...
  $search=$_GET['search'];
  $select_search_0=mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND discount_type='0' AND (`name` LIKE'%$search%' OR `short_description` LIKE '%$search%' OR `description` LIKE'%$search%')");
  $count_0=mysqli_num_rows($select_search_0);
  if($count_0>0 ){
    while($row_search_0=mysqli_fetch_assoc($select_search_0)){
?>
                    <?php
          $product_id=$row_search_0['id'];
              $check = mysqli_query($connect, "SELECT * FROM wishlist WHERE email='".$_SESSION['email']."' AND product_id='$product_id'");
              $inWishlist = mysqli_num_rows($check) > 0;
          ?>

                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_search_0['id']?>">
                            <img src="images/<?php echo $row_search_0['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_search_0['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR:
                                <?php echo $row_search_0['price']?> </p>
                        </a>
                        <div class="absolute top-2 right-2 flex flex-col space-y-2">
                            <form method="POST" action="wishlist-add.php">
                                <input type="hidden" name="product_id" value="<?php echo $product_id; ?>">
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_search_0['id']; ?>">
                                <button
                                    class="bg-white p-2 rounded-full shadow <?php echo $check_cart_c?'text-orange-600' : 'text_black hover:text-orange-600';?>"><i
                                        class="fas fa-cart-plus"></i></button>
                            </form>
                    ...
                    <?php
$select_search_1=mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND discount_type='1' AND (`name` LIKE'%$search%' OR `short_description` LIKE '%$search%' OR `description` LIKE'%$search%')");
$count_1=mysqli_num_rows($select_search_1);
  if($count_1>0 ){
    while($row_search_1=mysqli_fetch_assoc($select_search_1)){
?>
                    <?php
          $product_id=$row_search_1['id'];
              $check = mysqli_query($connect, "SELECT * FROM wishlist WHERE email='".$_SESSION['email']."' AND product_id='$product_id'");
              $inWishlist = mysqli_num_rows($check) > 0;
          ?>

                    <?php
            $price=$row_search_1['price'];
            $per=$row_search_1['discount_value'];
            $dis=($price*$per)/100;
            $final_pr=$price-$dis;
            ?>

                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_search_1['id']?>">
                            <img src="images/<?php echo $row_search_1['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_search_1['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR: <?php echo $final_pr;?> <span
                                    class="line-through text-gray-400 text-xs">PKR-<?php echo $row_search_1['price']?></span><span
                                    class="ml-2 bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-sm">-<?php echo $row_search_1['discount_value']?>%</span>
                            </p>
                        </a>
                        <div class="absolute top-2 right-2 flex flex-col space-y-2">
                            <form method="POST" action="wishlist-add.php">
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_search_1['id']; ?>">
                                <button
                                    class="bg-white p-2 rounded-full shadow <?php echo $check_cart_c?'text-orange-600' : 'text_black hover:text-orange-600';?>"><i
                                        class="fas fa-cart-plus"></i></button>
                            </form>
                    ...
                    <?php
$select_search_2=mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND discount_type='2' AND (`name` LIKE'%$search%' OR `short_description` LIKE '%$search%' OR `description` LIKE'%$search%')");
$count_2=mysqli_num_rows($select_search_2);
  if($count_2>0 ){
    while($row_search_2=mysqli_fetch_assoc($select_search_2)){
?>
                    <?php
          $product_id=$row_search_2['id'];
              $check = mysqli_query($connect, "SELECT * FROM wishlist WHERE email='".$_SESSION['email']."' AND product_id='$product_id'");
              $inWishlist = mysqli_num_rows($check) > 0;
          ?>

                    <?php
            $price=$row_search_2['price'];
            $per=$row_search_2['discount_value'];
            $final_pr=$price-$per;
            ?>

                    <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                        <a href="product.php?id=<?php echo $row_search_2['id']?>">
                            <img src="images/<?php echo $row_search_2['image']?>"
                                class="mx-auto mb-3 w-full h-48 object-cover">
                            <h3 class="font-semibold text-sm md:text-base"><?php echo $row_search_2['name']?></h3>
                            <p class="text-orange-600 font-bold text-sm md:text-base">PKR: <?php echo $final_pr;?> <span
                                    class="line-through text-gray-400 text-xs">PKR-<?php echo $row_search_2['price']?></span><span
                                    class="ml-2 bg-orange-100 text-orange-600 px-2 py-0.5 rounded text-sm">-<?php echo $row_search_2['discount_value']?>%</span>
                            </p>
                        </a>
                        <div class="absolute top-2 right-2 flex flex-col space-y-2">
                            <form method="POST" action="wishlist-add.php">
          ...
          ?>
                            <form method="POST" action="cart-add.php">
                                ...
                                <input type="hidden" name="product_id" value="<?php echo $row_search_2['id']; ?>">
                                <button
                                    class="bg-white p-2 rounded-full shadow <?php echo $check_cart_c?'text-orange-600' : 'text_black hover:text-orange-600';?>"><i
                                        class="fas fa-cart-plus"></i></button>
                            </form>
 ...
 echo '
       <div class="w-full flex items-center justify-center min-h-[60vh] col-span-full">
      <div class="bg-white shadow-lg rounded-lg p-8 text-center max-w-md w-full">
        <div class="text-6xl font-bold text-orange-600 mb-4">
          <i class="fas fa-exclamation-circle"></i>
        </div>
        <h1 class="text-2xl font-bold mb-2 text-gray-800">No Products Found</h1>
        <p class="text-gray-600 mb-6">Sorry, we couldn’t find any products matching your search.</p>
        <a href="index.php" class="inline-block bg-orange-600 text-white px-6 py-3 rounded hover:bg-orange-700">
          Back to Home
        </a>
      </div>
    </div>';
...
?>
                    <?php
        $select_product=mysqli_query($connect,"SELECT * FROM product WHERE status='1' AND discount_type='0'");
