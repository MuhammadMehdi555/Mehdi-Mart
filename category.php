...
$type=$_GET['type'];
if(empty($type)){
    // Invalid type, redirect ya message
    echo "<script>alert('Invalid category!'); window.location.href='index.php';</script>";
    exit();
}
$select_cate=mysqli_query($connect,"SELECT * FROM `sub-category` WHERE `parent-category`='$type'");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Mehdi Mart - Categories</title>
...
$wishlist_count = $row_wish['total'];
    ...
    <section class="md:col-span-3">
        ...
        <div class="flex justify-between items-center mb-6">
            <?php 
      $select_par=mysqli_query($connect,"SELECT * FROM `sub-category` WHERE `parent-category`='$type'");
      $row_par=mysqli_fetch_assoc($select_par);?>

            <h2 class="text-xl font-bold">All <?php echo $row_par['parent-category']?> Sub Categories</h2>

        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
            <?php 
        while($row_sub=mysqli_fetch_assoc($select_cate)){
          if($row_sub['status']==1){
        ?>
            <a href="shop.php?id=<?php echo $row_sub['id']?>">
                <div class="bg-white p-4 shadow rounded text-center relative hover:shadow-lg transition">
                    <img src="images/<?php echo $row_sub['image']?>" class="mx-auto mb-3 w-full h-48 object-cover">
                    <p class="text-orange-600 font-bold"><?php echo $row_sub['name']?></p>
