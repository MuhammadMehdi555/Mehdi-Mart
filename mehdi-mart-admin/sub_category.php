...
if(empty($_FILES['image']['name'])){
  ...
  if($move){
    $insert=mysqli_query($connect,"INSERT INTO `sub-category` (`image`,`name_parentname`, `name`, `parent-category`, `status`)
VALUES ('$new_name','$sub_name $parent_name', '$sub_name', '$parent_name', '0')
");
    if($insert){
           echo "<script>
          alert('✅ Great! Your Sub category has been added successfully.');
          window.location.href='sub_category.php';
        </script>";
    }
    else{
            echo "<script>
              alert('❌ Oops! Something went wrong, Sub category could not be added.');
          window.location.href='sub_category.php';
        </script>";
    }
  }
...
<head>
  ...
  <title>Mehdi Mart - Add Sub Category</title>
  ...
  <aside class="fixed top-0 left-0 h-full w-64 bg-white shadow-md hidden md:block">
    ...
    <div class="h-[calc(100%-80px)] overflow-y-auto">
        ...
        <a href="slider.php" class="block py-3 px-6 hover:bg-orange-100 rounded flex items-center gap-3"><i class="fas fa-image w-5"></i> Add Slider</a><a href="add-category.php" class="block py-3 px-6 hover:bg-orange-100 rounded flex items-center gap-3"><i class="fas fa-folder"></i> Add Category</a>
                <a href="sub_category.php" class="block py-3 px-6   bg-orange-100 rounded flex items-center gap-3"><i class="fas fa-folder-open w-5"></i> Sub Categories</a>
  ...
  <div class="md:ml-64 p-6">
    ...
    <div class="bg-white shadow rounded p-6 mb-6">
      ...
      <form class="grid grid-cols-1 md:grid-cols-2 gap-4" method="POST" enctype="multipart/form-data">
        ...
        <!-- Sub Category Name -->
        <div>
          <label class="block text-sm font-semibold text-gray-600 mb-2">Sub Category Name</label>
          <input type="text" name="name" class="w-full border p-2 rounded" placeholder="Enter sub category name" />
        </div>

        <!-- Parent Category -->
        <div class="md:col-span-2">
          <label class="block text-sm font-semibold text-gray-600 mb-2">Parent Category</label>
<select name="parent_id" required class="w-full border p-2 rounded">
    <?php
    $chack_category = mysqli_query($connect,"SELECT * FROM category");
    while($row_category = mysqli_fetch_assoc($chack_category)){
      if($row_category['status']==1){
    ?>
        <option value="<?php echo $row_category['name']; ?>">
    ...
    <?php } } ?>
        ...
        <div class="md:col-span-2">
          <button type="submit" name="save" class="px-4 py-2 bg-orange-600 text-white rounded hover:bg-orange-700">
            <i class="fas fa-upload mr-2"></i> Upload Sub Category
          </button>
        </div>
    ...
    <!-- LIST OF SUB CATEGORIES (Optional) -->
    <div class="bg-white shadow rounded p-6">
      <h2 class="text-xl font-bold mb-4">All Sub Categories</h2>
...
?>
        <div class="border rounded shadow hover:shadow-lg transition">
          <img src="../images/<?php echo $row_sub['image']?>" class="w-full h-40 object-cover rounded-t">
          <div class="p-4">
            <p class="text-gray-700 text-sm mb-2"><?php echo $row_sub['name']?></p>
            <p class="text-gray-700 text-sm mb-2"><?php echo $row_sub['parent-category']?></p>
            <div class="flex gap-4">
              
              <?php if($row_sub['status']==0){?>
              <a href="status.php?id=<?php echo$row_sub['id']?>&type=sub_category&status=1" class="flex-1 text-center py-3 bg-green-600 text-white rounded-lg text-base font-semibold shadow hover:bg-green-700 transition">
                Active
              </a>
              <?php } else{?>
              <a href="status.php?id=<?php echo$row_sub['id']?>&type=sub_category&status=0" class="flex-1 text-center py-3 bg-yellow-600 text-white rounded-lg text-base font-semibold shadow hover:bg-yellow-700 transition">
                Close
              </a>
              <?php } ?>

              <a href="delete.php?type=sub_category&id=<?php echo $row_sub['id']?>" class="flex-1 text-center py-3 bg-red-600 text-white rounded-lg text-base font-semibold shadow hover:bg-red-700 transition">
                Delete
              </a>
            </div>
          </div>
