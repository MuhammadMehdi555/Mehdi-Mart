...
if(isset($_POST['save'])){
    ...
    exit();
        ...
        if($move){
            $insert=mysqli_query($connect,"INSERT INTO category(`image`,`name`,`status`)VALUES('$new_name','$name','0')");
            if($insert){
     echo "<script>
          alert('✅ Great! Your category has been added successfully.');
          window.location.href='add-category.php';
        </script>";
            }
            else{
        echo "<script>
          alert('❌ Oops! Something went wrong, category could not be added.');
          window.location.href='add-category.php';
        </script>";
            }
        }
  ...
  <aside class="fixed top-0 left-0 h-full w-64 bg-white shadow-md hidden md:block">
    ...
    <div class="h-[calc(100%-80px)] overflow-y-auto">
      <nav class="mt-2">
        ...
        <a href="slider.php" class="block py-3 px-6 hover:bg-orange-100 rounded flex items-center gap-3"><i class="fas fa-image w-5"></i> Add Slider</a><a href="add-category.php" class="block py-3 px-6 bg-orange-100 rounded flex items-center gap-3"><i class="fas fa-folder"></i> Add Category</a>
  ...
  <div class="md:ml-64 p-6">
    ...
    <div class="bg-white shadow rounded p-6 mb-6">
      ...
      <form class="grid grid-cols-1 md:grid-cols-2 gap-4" method="POST" enctype="multipart/form-data">
        ...
        <div>
          ...
          <input type="text" name="name" class="w-full border p-2 rounded" placeholder="Enter category name" />
        </div>
        <div class="md:col-span-2">
          <button type="submit" name="save" class="px-4 py-2 bg-orange-600 text-white rounded hover:bg-orange-700">
            <i class="fas fa-upload mr-2"></i> Upload Category
          </button>
        </div>
      </form>
    </div>
    ...
    <div class="bg-white shadow rounded p-6">
      ...
      <div class="border rounded shadow hover:shadow-lg transition">
          ...
          <div class="p-4">
            <p class="text-gray-700 text-sm mb-2"><?php echo $row['name']?></p>
            <div class="flex gap-4">
              <?php if($row['status']==0){?>
                <a href="status.php?id=<?php echo $row['id']?>&type=category&status=1" 
                ...
                <?php } else{?>
                <a href="status.php?id=<?php echo $row['id']?>&type=category&status=0" 
                  ...
                  Close
                    ...
                    <?php }?>
              <a href="delete.php?id=<?php echo $row['id']?>&type=category" 
